...
const OUTPUT_SCHEDULE = `One <new_schedule> per character PRESENT in the latest exchange (plus {{user}} as "User") whenever their schedule for the CURRENT clock date is missing - either because the clock crosses into a NEW calendar date, or because <chronogram_state> marks the character with NO CHRONOGRAM (e.g. after a reset or rollback; rebuild it immediately, same run). Characters that DO have a schedule for the current date are skipped entirely. Characters absent from the exchange are off-screen: skip them too (they rejoin automatically when they return).
<new_schedule>
Owner:Aldric
Date:00/00/0000
07:00 wakes up
...
// The request is split into three explicit regions - CONTEXT (reference
// only), FROM (the already-tracked anchor) and TO (the new exchange) - so
// the model never blends older history into the delta it must report.
const INPUT_STRUCTURE = `## INPUT
Each request is split into three clearly separated regions:
1. CONTEXT - this instruction, optional <story_info> reference data, then <conversation_context> (older history, REFERENCE ONLY) and the <last_tracked_turn> block: the FROM anchor, the PREVIOUS exchange, ALREADY tracked. The <chronogram_state> block follows it and describes the world (clock, schedules, objectives) at the END of that FROM exchange.
2. TO - as the VERY LAST message, the <exchanges_to_analyze> block wrapping <last_turn>: the NEW exchange, NOT yet tracked. This is the ONLY exchange you analyze and the ONLY source of story time. Advance from the state in <chronogram_state> based ONLY on what happens in <last_turn>.`;

const NORMAL_RULES = `## TRACKING
Compare FROM to TO: the clock stands at the END of <last_tracked_turn> (as shown in <chronogram_state>); measure ONLY what happens inside <last_turn> and emit it as the clock_update delta (Days/Hours/Minutes). Never count earlier turns or context history as elapsed story time. DEFAULT TO MINUTES: a normal exchange is 1-5 minutes; Hours only for an explicit time skip (travel, sleep, waiting, scene break), Days only for an explicit day transition. EVERY character marked PRESENT in <chronogram_state> must end the run with a schedule for the date AFTER the advance: emit <new_schedule> when the clock crosses into a NEW calendar date OR when a character is marked NO CHRONOGRAM - a new schedule's Date: is always that post-advance date. Maintain objectives. A quiet continuation still emits clock_update (it may be all zeros). Leave off-screen characters untouched.`;
