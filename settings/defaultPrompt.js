...
Each request arrives as: (1) this instruction, (2) optional <story_info> reference data, (3) the conversation context (<conversation_context> or role turns), which contains a <last_tracked_turn> block (the PREVIOUS exchange, ALREADY tracked - the world clock stands at its end) and ends with the <exchanges_to_analyze> block (the NEW exchange, NOT yet tracked), and (4) as the VERY LAST message, <chronogram_state>: the current tracked state (world clock, PRESENT characters' schedules, objectives). Treat <chronogram_state> as the authoritative starting point to advance from.`;

const NORMAL_RULES = `## TRACKING
Using Time Passed and what happened, decide how much story time passes and emit it as the clock_update delta (Days/Hours/Minutes). Measure the delta from the END of <last_tracked_turn> (where the clock stands) to the END of <exchanges_to_analyze>: only what happens in the new exchange adds time. DEFAULT TO MINUTES: a normal exchange is 1-5 minutes; Hours only for an explicit time skip (travel, sleep, waiting, scene break), Days only for an explicit day transition. When the delta crosses into a NEW calendar date, generate schedules - a new schedule's Date: is the NEW date AFTER the advance. Maintain objectives. A quiet continuation still emits clock_update (it may be all zeros). Leave off-screen characters untouched.`;
