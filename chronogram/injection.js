...
export function buildInjectionText() {
    ...
    // Characters disabled -> no presence pass and no <person> blocks at all.
    const participants = trackCharacters ? getActiveParticipants() : {};
    if (trackCharacters) updateActiveParticipants();
    ...
    if (active.length > 0) {
        ...
        for (const o of active) {
            const ownerName = o.owner === "user" ? "User" : (allParticipants[o.owner]?.name || o.owner);
...
function renderParticipantCard(id, p) {
    ...
    const schedLines = sched.map(e => (
        ...
        + `<span class="chrono-sched-time">${escapeHtml(e.time)}</span>${escapeHtml(stripCurrentMarker(e.activity))}</div>`
    )).join("");
...
export function renderPanelHTML() {
    ...
    // Presence pass so the drawer mirrors exactly who is in the current context.
    if (trackCharacters) updateActiveParticipants();

    const clock = getClock();
    const participants = trackCharacters
        ? Object.entries(getParticipants()).filter(([, p]) => p?.active !== false)
        : [];
    const objectives = trackObjectives ? getObjectives() : [];

    if (!clock && (participants.length === 0 || !trackCharacters)) {
